 ...
 */
	function initialize() {
		$this->dirsToChmod[] = WWW_ROOT . 'files' . DS . 'profile_media' . DS . '*';
	}
/**
 ...
 */
	function chmod() {
		foreach ($this->dirsToChmod as $dir) {
			$count = 0;
			$path = r(DS . '*', '', $dir);
			if (substr($dir, -2) != DS . '*') {
				chmod($dir, 0765);
				...
				foreach ($contents[0] as $subdir) {
					chmod($path . DS . $subdir, 0765);
					$count++;
				}
			}
		}
		$this->out('-> ' . $count . ' folders chmodded');
	}
/**
