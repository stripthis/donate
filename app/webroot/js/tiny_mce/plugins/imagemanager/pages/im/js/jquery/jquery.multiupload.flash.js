	...
	if (!$.multiUpload.initialized && getFlashVersion() >= 10) {
		...
		$.extend($.multiUpload.prototype, {
			...
			init : function() {
				...
				$(document.body).append('<div id="flashUploaderContainer"><div id="flashreplace"></div></div>');

				swfobject.embedSWF("js/jquery/jquery.multiupload.flash.swf", "flashreplace", "100%", "100%", "10.0", null, {
					file_filter : $.map(up.settings.filter, function(v) {
						return '*.' + v
					}).join(';')
				}, {
					wmode : "transparent"
